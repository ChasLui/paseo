...
/**
 * The daemon inherits whatever environment its launcher had — a launchd job, a
 * bare `paseo daemon start` from a minimal shell, or the desktop app. When that
 * environment is minimal (no login PATH, wrong or missing SHELL), every
 * terminal and agent process the daemon spawns inherits the same breakage
 * (e.g. terminals falling back to a bare /bin/bash on macOS).
 *
 * Spawn the user's login shell and capture its full environment via Node's
 * JSON.stringify(process.env), so the daemon and all child processes see the
 * same tools and variables as a normal terminal session.
 *
